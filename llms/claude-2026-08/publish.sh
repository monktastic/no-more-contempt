#!/bin/sh
# Generates the website pages from long-book/: index.md, book/, map/, appendix/,
# short/. They go to the repo root, where Jekyll finds them, and are not in
# git; the Actions build regenerates them on every push. Edit long-book/ instead.
...
MAN  = HERE / 'long-book'
GEN  = '<!-- Generated by llms/claude-2026-08/publish.sh from long-book/. Do not edit; edit the source. -->\n\n'

# Pages are collected first and written at the end, so that a page in a reading
# sequence can link to the one before and the one after it. seq names the
# sequence; hub pages that only list their children belong to none.
pending = []
def page(path, title, order, permalink, body, parent=None, has_children=False, grand_parent=None, seq=None):
    pending.append(dict(path=path, title=title, order=order, permalink=permalink, body=body,
                        parent=parent, has_children=has_children, grand_parent=grand_parent, seq=seq))

def write(path, title, order, permalink, body, parent=None, has_children=False, grand_parent=None,
          prev=None, next=None, **_):
    ...
    if prev: fm += [f'prev_title: "{prev["title"]}"', f'prev_url: {prev["permalink"]}']
    if next: fm += [f'next_title: "{next["title"]}"', f'next_url: {next["permalink"]}']
    fm.append('---')
...
# that edit was made in the wrong place and would be lost. Port it to long-book/
...
if GUARDED and MANIFEST.exists() and not os.environ.get('PUBLISH_FORCE'):
    ...
    if edited:
        sys.exit('publish.sh: these generated pages were edited after the last publish; port the edits to '
                 'long-book/ first, or PUBLISH_FORCE=1 to discard them:\n  ' + '\n  '.join(edited))
written = {}
...
page(ROOT / 'index.md', 'Start here', 1, '/', body, seq='book')
...
for i, (f, override) in enumerate(chapters, 1):
    ...
    if body.startswith('<p '):  # the draft banner stays first
        ...
        body = line + body
    page(ROOT / 'book' / f, override or t, i, f'/book/{f[:-3]}/', body, parent='The Book', seq='book')
